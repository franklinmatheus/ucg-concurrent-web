...
public class PopulateDatabaseBusiness {
    ...
    public void generateNewClients(int quantity) {
        ...
        List<Populater> populaters = new ArrayList<>();
        
        for (int i = 0; i < slice; ++i) {
            Populater populater = new Populater(i, Common.THREAD_POPULATE_WORK);
            populaters.add(populater);
        }
        
        for (int i = 0; i < slice; ++i) {
            populaters.get(i).start();
        }
        
        for (int i = 0; i < slice; ++i) {
            try {
                populaters.get(i).join();
